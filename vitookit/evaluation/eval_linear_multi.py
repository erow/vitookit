...
def eval_linear(args):
    ...
    valdir=os.path.join(args.data_location, "val")
    ...
    for pm in args.permutes:
        ...
        optimizer = torch.optim.SGD if optim == 'sgd' else misc.LARS
        optimizer = optimizer(
            parameters,
            lr * (args.batch_size_per_gpu * misc.get_world_size()) / 256., # linear scaling rule
            momentum=0.9,
            weight_decay=wd,
        )
    ...
    for inp, target in metric_logger.log_every(val_loader, 20, header):
        ...
        for group, linear_classifier in enumerate(linear_classifiers):
            ...
            if linear_classifier.module.num_labels >= 5:
                acc1, acc5 = misc.accuracy(pred, target, topk=(1, 5))
                acc1s.append(acc1)
                acc5s.append(acc5)
            else:
                acc1, = misc.accuracy(pred, target, topk=(1,))
                acc1s.append(acc1)
...
def get_args_parser():
    ...
    parser.add_argument('--data_location', default='/path/to/imagenet/', type=str,
